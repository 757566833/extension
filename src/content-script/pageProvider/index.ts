...
const log = (event, ...args) => {
  if (process.env.NODE_ENV !== 'production') {
    console.log(
      `%c [anysat] (${new Date().toTimeString().slice(0, 8)}) ${event}`,
...
const channelName = script?.getAttribute('channel') || 'ANYSAT';
...
if (!window.anysat) {
  window.anysat = new Proxy(provider, {
    deleteProperty: () => true
  });
}

Object.defineProperty(window, 'anysat', {
...
window.dispatchEvent(new Event('anysat#initialized'));
